    ...
    if (moduleInfo.id === 'memorization') {
      ...
      task.ranges.forEach((r) => {
        if (r.type === 'page') {
          ...
          // Approximate logic: this requires tracking by Ayah or simply marking the surah range as memorized.
          ...
          {activeSelections.length === 0 ? (
            ...
            activeSelections.map((task) => (
              <View key={task.id} style={styles.taskCard}>
                ...
                <TouchableOpacity
                  style={[styles.completeBtn, { backgroundColor: `${moduleInfo.color}15` }]}
                  onPress={() => handleComplete(task)}
                >
                  <Ionicons name="checkmark-circle" size={20} color={moduleInfo.color} />
                  <Text style={[styles.completeBtnText, { color: moduleInfo.color }]}>
                    تحديد كمنجز
                  </Text>
                </TouchableOpacity>
              </View>
          ...
          <View style={[styles.section, { marginTop: Spacing.xl }]}>
            ...
            {completedSelections.map((task) => (
              <View key={task.id} style={[styles.taskCard, { opacity: 0.6 }]}>
                <View style={styles.taskHeader}>
                  <Text style={styles.taskDate}>
                    تم الإنجاز: {task.completedAt ? new Date(task.completedAt).toLocaleDateString('ar-EG') : ''}
                  </Text>
                  <Ionicons name="checkmark-done" size={16} color={Colors.success} />
                </View>
                <View style={[styles.rangesContainer, { pointerEvents: 'none' }]}>
                  {task.ranges.map((r, i) => (
                    <RangeChip key={i} range={r} onRemove={() => {}} />
                  ))}
                </View>
